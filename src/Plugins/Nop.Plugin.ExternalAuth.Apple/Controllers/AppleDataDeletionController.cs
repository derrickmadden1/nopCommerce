...
public class AppleDataDeletionController : Controller
{
    #region Fields
    ...
    public AppleDataDeletionController(AppleExternalAuthSettings AppleExternalAuthSettings,
    ...
    public async Task<IActionResult> DataDeletionCallback(IFormCollection form)
    {
        try
        {
            ...
            var appSecretBytes = Encoding.UTF8.GetBytes(_AppleExternalAuthSettings.PrivateKey);
            ...
            var fbUser = JsonConvert.DeserializeObject<AppleUserDTO>(json);
            ...
            {
                await _logger.InformationAsync($"{AppleAuthenticationDefaults.SystemName} data deletion completed. " +
            ...
            var notificationUrl = Url.RouteUrl(AppleAuthenticationDefaults.DataDeletionStatusCheckRoute,
        ...
        {
            await _logger.ErrorAsync($"{AppleAuthenticationDefaults.SystemName} data deletion error: {exception.Message}.", exception);
            return BadRequest();
        }
    }

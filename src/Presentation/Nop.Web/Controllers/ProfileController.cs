...
public partial class ProfileController : BasePublicController
{
    ...
    public virtual async Task<IActionResult> Index(int? id, int? pageNumber)
    {
        ...
        if (await _permissionService.AuthorizeAsync(StandardPermission.Security.ACCESS_ADMIN_PANEL) && await _permissionService.AuthorizeAsync(StandardPermission.Customers.CUSTOMERS_CREATE_EDIT_DELETE))
